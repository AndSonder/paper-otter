...
4. 实际打开图片；PDF/EPS 原图需用可用工具渲染。对照同版 PDF 或官方 HTML，确认图号、子图、坐标、图例和适用条件。若从 PDF 裁图，先看完整页面定位边界，再直接从 PDF 高分辨率渲染目标矩形；禁止凭旧裁图的位置猜坐标。仅在人工核对后记录 verified，并保留页码、裁切框或原始候选索引。
...
6. 定稿前将全部成品图做成联系表复查。任何一张图出现 caption/正文残片、边缘断线、缺失标签、图号错配或在栏宽下无法辨认，都视为未完成，修正后重新检查，不能以“主体大致截到”通过。
