...
export default function Reader() {
  ...
  if (!paper) return <div className="app">
    ...
    <main className="setup-empty"><span className="eyebrow">PERSONAL READING SYSTEM</span><h1>{catalogReady ? "先建立你的阅读画像" : "正在读取本地阅读空间…"}</h1>{catalogReady && <><p>这个仓库不预装论文。使用 <code>$daily-paper-recommender</code> 回答几组引导问题，大模型会在本地建立画像，并从你的目标、阅读行为和明确反馈中学习。</p><ol><li>初始化关注的问题、已有知识与阅读习惯</li><li>让 skill 推荐今天的一篇主读和少量备选</li><li>运行 <code>python3 scripts/paperctl.py sync</code>，回到这里阅读</li></ol><div className="setup-command">$daily-paper-recommender 初始化我的阅读系统</div></>}</main>
  </div>;
